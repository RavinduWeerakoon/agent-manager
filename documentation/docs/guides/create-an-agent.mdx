...
This is the field-by-field reference for the **Add Agent** flow. For a narrative
walkthrough that deploys a real agent end to end, start the tutorial series at
[Deploy the IT Helpdesk Agent](../tutorials/create-your-first-agent.mdx).
...
The choice is fixed at creation — see [Internal and External Agent](../concepts/internal-and-external-agent.mdx) for what each implies.
...
Click **Register** to complete the registration. The agent will appear in your agent list and start receiving observability data once you instrument it using the [AMP instrumentation](./amp-instrumentation.mdx) package.
...
- **Agent Catalog** — Pick a pre-built agent kind from the [Agent Catalog](../concepts/agent-kind-and-catalog.mdx).
...
Click **+ Add** to bind one or more LLM providers to this agent. LLM providers are configured separately — see [Register an LLM Service Provider](./register-llm-service-provider.mdx).
...
- [Agent and Agent Life Cycle](../concepts/agent-lifecycle.mdx) — what happens after creation
- [AMP Instrumentation](./amp-instrumentation.mdx) — instrumenting an externally-hosted agent
- [Configure Agent LLM Configuration](./configure-agent-llm-configuration.mdx) — attaching a provider later
