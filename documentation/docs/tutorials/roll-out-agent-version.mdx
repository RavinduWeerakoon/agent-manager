...
You need at least two to see a promotion happen. Keep the sandboxing mode as the default as per this tutorial when creating the new enviroments

After creating the enviroments go to the Deployement Pipelines view at the orgnization page and then click edit pipeline if the two newly created enviroments ar not visible in the promotion chain update the promotion chain with Default, staging and production respectivly and click save


## Step 2: Promote to Staging

Go to the the agent's deployment view, promote the development deployment to staging.
...
- **MCP proxy endpoint** — Will used different access token and the github orgnaization might be changed.
- **`AGENT_VERSION`** — set it to something distinguishable, like `1.0.0-staging`.

So before promoting the deployemnt across enviroments you have to config the LLM service provider and the MCP proxy endpoints for the newly created enviorments and as  mentioned earlier attach the same provider and the proxy for the new enviroments as well

Now go to the deploy page and 
