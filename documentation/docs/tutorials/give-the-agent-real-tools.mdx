...
Back on the `it-helpdesk` agent page go to the configure page and then go to tool configuration and attach the newly registered MCP to the it helpdesk agent. Change the enviroment variable names to `GITHUB_MCP_URL` and `GITHUB_MCP_API_KEY`.
...
repository while production points at the real tracker, using the same
...
the running agent won't see the new tools until it restarts, which means a redeploy for
