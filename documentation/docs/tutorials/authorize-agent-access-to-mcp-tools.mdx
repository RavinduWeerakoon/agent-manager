...
- `amctl` installed and logged in against your control plane (see [CLI Installation](../guides/cli-installation.mdx) and [`amctl login`](../reference/cli/login.mdx)).
- An AI Gateway registered and active for the environment you are configuring (see [Register an AI Gateway](../guides/register-ai-gateway.mdx)).
...
You can also register a proxy through the Console — see [Register an MCP Proxy](../guides/register-mcp-proxy.mdx) — but the rest of this tutorial uses the API for the parts that define authorization.
...
- [Register an MCP Proxy](../guides/register-mcp-proxy.mdx) — the full proxy registration reference, including the Console flow.
- [Configure Agent MCP Proxies](../guides/register-mcp-proxy.mdx#configure-an-agent-to-use-a-proxy) — attach a proxy to an agent so it can call the tools.
