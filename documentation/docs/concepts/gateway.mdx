...
A gateway is registered at the organization level, but it deploys one artifact per **[Environment](./environment.mdx)** it's active in. When you register an LLM Service Provider or MCP Proxy, the platform deploys that resource's configuration into each environment it's configured for, onto an egress gateway active in that environment — you don't wire a provider to a gateway environment by environment yourself. You only name a gateway explicitly when an environment offers more than one choice; see [How the Active Gateway Is Selected](#how-the-active-gateway-is-selected).
...
### How the Active Gateway Is Selected
