...
**[LLM Service Providers](./llm-service-provider.mdx)** and **[MCP Proxies](./agent-tools.mdx)** are defined at the organization level rather than inside a project, so any agent in any project can reference them.

**[Environments](./environment.mdx)** and **[Gateways](./gateway.mdx)** are organization-level, with each environment having its own gateway. Identity providers are attached to those gateways, and determine which token issuers each environment trusts.
