...
A provider is defined once at the organization level, but it isn't reachable until it's deployed to a [Gateway](./gateway.mdx). The platform deploys one gateway artifact per [environment](./environment.mdx) the provider is configured for, onto an egress gateway active in that environment. This is the same define-once/deploy-per-environment pattern the rest of the platform follows: one provider record, several running exposures of it.
