...
Creating an agent from a published kind is an alternative to pointing at source code directly (see [Choose a Source Type](../guides/create-an-agent.mdx#step-1-choose-a-source-type) in Create an Agent). Either path produces the same kind of running [agent](./agent-lifecycle.mdx); a kind just captures "build once, deploy many times" so a proven agent doesn't need to be rebuilt from scratch for every reuse.
