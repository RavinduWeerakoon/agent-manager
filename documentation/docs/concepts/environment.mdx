...
An Environment is a stage your agents run in — development, staging, and production being the typical examples. Every agent deployment, gateway registration, and MCP proxy endpoint in Agent Manager is scoped to one or more environments.
...
- **Gateway artifacts** — an [LLM Service Provider](./llm-service-provider.mdx) or [MCP Proxy](./agent-tools.mdx) deploys one gateway artifact per environment it's configured for, to that environment's active **[Gateway](./gateway.mdx)**.
