...
| `agentManagerService.config.fileMountLimits` | File mount size caps, in bytes of inline content (secret-reference mounts do not count). All of an agent's mounts render into one ConfigMap/Secret, which Kubernetes limits to 1 MiB, so maxTotalBytes may not exceed 1048576 and maxFileBytes may not exceed maxTotalBytes. | object |  |
| `agentManagerService.config.fileMountLimits.maxFileBytes` | Largest inline content a single file mount may carry. | integer | `1000000` |
| `agentManagerService.config.fileMountLimits.maxTotalBytes` | Combined inline content of all file mounts in one request. | integer | `1000000` |
...
| `console.config.maxRequestBodyBytes` | Largest request body, in bytes, the console sends on a save. Bigger saves are refused with a readable error instead of reaching a WAF that would reject them with an opaque 403. The default suits a 64 KB WAF body limit; set 0 on deployments with no such limit. | integer | `57344` |
| `console.config.fileMountMaxFileBytes` | Per-file cap, in bytes, on file-mount content in the console. 0 uses agentManagerService.config.fileMountLimits.maxFileBytes, keeping the two in step. | integer | `0` |
