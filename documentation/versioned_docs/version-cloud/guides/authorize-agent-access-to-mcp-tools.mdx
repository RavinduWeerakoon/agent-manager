...
- `amctl` installed and logged in against your organization (see [CLI Installation](./cli-installation.mdx), then `amctl login`).
...
:::caution Changes apply to the next token, not to tokens already issued
Scopes are resolved when a token is minted, so a change to a role, its scopes, or
its assignments takes effect on the **next** token the agent requests. A token the
agent is already holding keeps the scopes it was minted with until it expires — up
to the access token lifetime configured for the environment, 3600 seconds by
default.

The same applies to credentials. Revoking or rotating an agent's AgentID secret
stops it minting **new** tokens, but does not invalidate tokens already issued:
those remain valid until they expire, because the identity provider cannot recall a
token it has already handed out (see
[Rotating and Revoking Credentials](../concepts/agentid.mdx#rotating-and-revoking-credentials)).

To cut off access immediately rather than waiting out the token lifetime, remove
the capability at the proxy — drop the tool from the endpoint's allowed
capabilities, so the gateway stops serving it regardless of what any outstanding
token claims.
:::
