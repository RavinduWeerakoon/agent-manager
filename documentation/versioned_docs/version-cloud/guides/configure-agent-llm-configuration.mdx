...
- The **upstream provider credential** (for example, your OpenAI key) is never given to the agent. It stays on the org-level provider, and the AI Gateway attaches it on the outbound call. What the agent receives instead is a platform-issued key scoped to this LLM configuration.
  - **Platform agents** read the endpoint and that key from the injected environment variables (`OPENAI_URL` and `OPENAI_API_KEY` in the example above). Agent code can read them like any other variable, so treat them as secrets — do not log them, echo them in responses, or write them to traces.
  - **External agents** receive the same gateway endpoint plus a configuration-specific client key, which you set wherever the agent runs.
