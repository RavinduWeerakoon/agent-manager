...
You can also attach MCP proxies while creating a new **Platform-Hosted** agent, without
visiting the Configure page afterward. This applies to both source-based and
catalog-based creation.

:::note Not available for External agents
The External agent creation form has no **MCP Proxies** section, and the variable
names below are the injection mechanism — which only applies to workloads the
platform runs. To give an External agent access to a proxy, register the agent
first, then follow
[Configuring an MCP Proxy for an External Agent](#configuring-an-mcp-proxy-for-an-external-agent).
:::
