...
- An Agent Manager Cloud account (sign up at [console.agent-manager.cloud.wso2.com](https://console.agent-manager.cloud.wso2.com/))
...
Source containing `os`, `subprocess`, `socket`, `ctypes`, or `importlib` imports (or a dynamic `__import__()` call) is rejected at creation/update time with a 400 error naming the pattern. This is a content check rather than a full sandbox, so write evaluators that don't need these regardless. It is not the only boundary: on Agent Manager Cloud evaluator code also runs with restricted network egress and cannot reach arbitrary external services — see [Network access](../concepts/evaluation.mdx#custom-evaluators).
:::
