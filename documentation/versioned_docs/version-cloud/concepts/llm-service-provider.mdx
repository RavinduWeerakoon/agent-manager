...
- Registering and changing providers is governed by RBAC. The **AI Lead** and **Admin** roles own the provider itself, including its global and resource-wise guardrails. A **Developer** can attach a provider to an agent and set **agent-level** guardrails on that binding, but cannot create, update, or delete the shared provider. See [Access Control](./organization.mdx#access-control).
