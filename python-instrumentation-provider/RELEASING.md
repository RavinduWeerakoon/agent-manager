...
2. **Pick the new AMP-instr semver.** Minor bump if there's a behaviour change (a new
   OpenLLMetry usually is); patch for trivial fixes — including an upstream patch bump.
   Say `0.4.0`.

   **Never re-point an already-published version at a different `traceloop_version`.**
   Once a version ships it is immutable in practice: PyPI permanently blocks re-uploading
   a version, so the `AMP Instrumentation Release` workflow would fail outright — and
   `release.yml` rebuilds *every* `release-config.json` entry on each product release, so
   editing an existing entry would silently overwrite its published
   `<version>-python<X.Y>` tags and move every agent pinned to it onto a different SDK.
   Always add a new entry.

   Also check whether the version you're about to claim is already used as a fixture in
   `agent-manager-service/instrumentation/testdata/extension_*.yaml` — those deliberately
   use a version *not* in the baseline, so cutting it for real means moving the fixture up.
...
4. **Init-container images** (`.github/release-config.json`): **add a new entry** to the
   ...
   ```
   No Dockerfile change needed for CI. (The `ARG TRACELOOP_VERSION` default in the
   `Dockerfile` and `TRACELOOP_VERSION ?=` in the `Makefile` only affect a bare local
   `make build`; bump them alongside so local builds don't quietly build the oldest
   version.)

   In the **same PR**, add the new Traceloop version to
   `test/instrumentation-matrix/matrix.yaml` — both `providers.traceloop.versions` and
   the `instrumentationVersions` map — or `make check-matrix-manifest` fails CI. Promote
   `defaultCell.providerVersion` to the new version too: that turns the new cell into a
   required PR job, so the PR itself gates on a real run against the new SDK.
...
6. **Regenerate the server's embedded catalog**. The `agent-manager-service` binary
   ...
   the chart's default from drifting.

   `agent-manager-service/instrumentation/catalog_test.go` asserts the exact number of
   baseline entries (`len(All)`, in two tests) and lists the expected versions, so a new
   entry fails the Go unit job until you update it. Merge the PR.
...
8. **Make it the platform default** (when you want *new* agents to get `0.4.0`).
   ...
   ```
   …**and** bump the matching in-code fallback in
   `agent-manager-service/config/config_loader.go`
   (`readOptionalString("OTEL_DEFAULT_INSTRUMENTATION_VERSION", …)`). Nothing tests this,
   and a stale value silently hands the old default to any deployment that doesn't set
   the env var.
