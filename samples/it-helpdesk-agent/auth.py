...
async def home():
    ...
    if user:
        try:
            access_token = await auth0().get_access_token(store_options={"request": request})
        except Exception as e:
            access_token = f"Error retrieving access token: {str(e)}"

        return f"""
            {head}
            <p>Logged in as {escape(user.get("email", ""))}</p>
            <h1>Bearer Token (Access Token)</h1>
            <textarea style="width: 100%; height: 100px; font-family: monospace;" readonly>{escape(access_token or '')}</textarea>
            <h1>User Profile</h1>
